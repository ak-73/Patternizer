//just straight up HACK by subclassing via copy and paste
//FIXME this just seemed like the fastest method at the time to customize the proposal behaviour and get an idea of how it works. i need to take more than a cursory look at how best to adapt it to my needs.
//FIXME sometimes replacement of widget text doesnt work right
//TODO single click to select a proposal
 ...
 *******************************************************************************/
package de.patternizer.eclipse.patterns.helpers.gui;
...
public class CustomContentProposalAdapter extends ContentProposalAdapter {
	...
	public class ContentProposalPopup extends PopupDialog {
		...
		public final class PopupCloserListener implements Listener {
			...
			public void handleEvent(final Event e) {
				...
				//System.out.println("Event: " + e.widget);
				if (e.type != SWT.MouseDown) close();
				else
				{
					if (e.widget != ContentProposalPopup.this.proposalTable) 
						{
							//Instant eventTime = Instant.ofEpochMilli((long) (e.time & 0xFFFFFFFFL));
							Instant handleEventTime = Instant.now();
							Duration dur = Duration.between(ContentProposalPopup.this.timestampOpen, handleEventTime);
							//System.out.println("Clicked outside of table");
							if (dur.getNano() >= 50000000)
							{
								//System.out.println("...and outside blocking time interval");
								close();
							}
						}
				}
		 ...
		 */
		public PopupCloserListener popupCloser;
		...
		public int open() {
			...
			if (value == OK) timestampOpen = Instant.now();
			if (popupCloser == null) {
		...
		java.time.Instant timestampOpen = null;
	 ...
	 */
	public ContentProposalPopup popup;
	 ...
	 */
	public CustomContentProposalAdapter(Control control,
